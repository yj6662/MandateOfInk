...
| 마지막 갱신 | 2026-07-20 |
| 다음 할 일 | D28 게이트 빌드 + 외부 5인 테스트 준비 (M1 마지막 항목) |
| 블로커 | 없음 |
...
- [~] D28. 수묵 룩 프로토타입 — **임시 코드 셰이더 스탠드인**(사용자 지시): ToonLitTemp(셀 3단, URP Lit 프로퍼티 호환=MPB 틴트 유지, 머티리얼 8종 전환) + InkPostTemp(URP Full Screen Pass·RG 네이티브 — 뎁스 먹 외곽선·한지 세피아·정적 그레인·비네트). Flat Kit+Snapshot Shaders Pro 구매 시 교체. 게이트용 빌드·외부 5인 테스트 잔여
