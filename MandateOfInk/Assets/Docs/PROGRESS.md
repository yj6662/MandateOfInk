...
| 마지막 갱신 | 2026-07-20 |
| 다음 할 일 | D21 잔여(졸개 텔레그래프 이식) → D22 한 합 검증 → W4(받아치기·그로기·상합) |
| 블로커 | 없음 |
...
- [~] D21. 졸개 텔레그래프(느린/빠른/페인트, 0.8~1.2초) — **보스 패턴으로 선행 구현**: BossAI 상태머신에 느린(팔 휘두르기 1.6s·진홍)/빠른(다연장 부채꼴 0.5s·주황)/페인트(0.7s 후 멈칫 0.55s→지연 타격) 3종 어휘. 졸개(EnemyAI)에의 이식은 잔여
