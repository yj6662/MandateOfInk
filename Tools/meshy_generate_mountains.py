# Meshy text-to-3d로 원산(遠山) 모델 2종 생성 — 인왕제색도풍 원경 배치용(사용자 결정 2026-07-27).
# A=화강암 주봉(인왕산풍 둥근 바위 봉우리), B=낮은 다봉 능선. preview -> refine -> FBX/텍스처 다운로드.
# 사용: python meshy_generate_mountains.py  (Tools/.env 의 MESHY_API_KEY 사용)
import io
import json
import os
import sys
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "MandateOfInk", "Assets", "_Project", "Art", "Models", "Mountains")

MOUNTAINS = [
    ("SM_Mountain_Peak", "Mountain_Peak",
     "Large Korean granite mountain, single massive rounded dome peak like Inwangsan, "
     "steep bare rock cliffs, weathered stone ridges, sparse low pine trees near base, "
     "natural terrain, game environment background asset, wide solid base"),
    ("SM_Mountain_Ridge", "Mountain_Ridge",
     "Long low mountain ridge with several gentle overlapping peaks, layered rocky hills, "
     "Korean mountain range landscape, soft undulating silhouette, natural terrain, "
     "game environment background asset, wide solid base"),
]
