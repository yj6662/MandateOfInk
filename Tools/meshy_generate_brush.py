...
OUT_DIR = os.path.join(os.path.dirname(__file__), "..", "MandateOfInk", "Assets", "_Project", "Art", "Models", "Brush")
PROMPT = ("Traditional Korean calligraphy brush (daepil), long dark wooden handle, "
          "black ink-soaked bristle tip, simple elegant shape, game prop, clean topology")
...
preview_id = call("POST", BASE, {
    ...
    "target_polycount": 8000, "topology": "triangle", "should_remesh": True,
})["result"]
...
download(fbx, os.path.join(OUT_DIR, "SM_Brush.fbx"))
for i, tex in enumerate(task.get("texture_urls", []) or []):
    for kind, url in tex.items():
        if url:
            download(url, os.path.join(OUT_DIR, f"T_Brush_{kind}.png"))
print("완료", flush=True)
